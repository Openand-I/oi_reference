...
/*
// Begin modification
        im_expand = img_new(w*2,h*2);
	unsigned char *pix = im->pix, *pixo = im->pix, *pix_expand = im_expand->pix;
        unsigned int offset = 0; // This was a 1, see if that was an error
//	for (int i = 0, pix = im->pix; i < im->w * im->h; i++, pix += 3) {
	for (int i = 0; i < im_expand->w * im_expand->h; i++, pix_expand += 3, offset++) {
            pix = pixo + ( ( offset / im_expand->w ) / 2 * im->w + ( offset % im_expand->w ) / 2 ) * 3;
            pix_expand[0] = pix[0];
            pix_expand[1] = pix[1];
            pix_expand[2] = pix[2];
         }
...
static __attribute__((always_inline)) inline int cmp_node(oct_node a, oct_node b)
{
...
//			v[2] = px[2] - nd->b;
                        ...
                        // This is a good original upgrade. Commenting for the client side random upgrade
//			v[0] = nd->r - px[0];
//			v[1] = nd->g - px[1];
//			v[2] = nd->b - px[2];
 
                        // This has potential as a good client side upgrade.
                        v[0] = ( rand() % 3 ) - 1;
                        v[1] = ( rand() % 3 ) - 1;
                        v[2] = ( rand() % 3 ) - 1;
...
unsigned short ByteCap(int num)
{
...
*/
	...
	for (i = 1; i < heap.n; i++) {
		...
		got->g = got->g / c + .5;
		got->b = got->b / c + .5;
	if ( dither == 2 ) /* Just write the color palette to file */
          {
                printf("%ld %ld %ld\n",got->r,got->g,got->b);
          } else
	if ( dither == 3 ) /* Just write the color palette to file */
          {
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-1),ByteCap(got->b-1));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g+1),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-1),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g+1),ByteCap(got->b-1));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g-1),ByteCap(got->b-1));
	...
	if ( dither == 4 ) /* Just write the color palette to file */
          {
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-1),ByteCap(got->b-1));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g+1),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-1),ByteCap(got->b+1));
                ...
                printf("%u %u %u\n",ByteCap(got->r-0),ByteCap(got->g-1),ByteCap(got->b-1));
                printf("%u %u %u\n",ByteCap(got->r+0),ByteCap(got->g+1),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r-0),ByteCap(got->g-1),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r-0),ByteCap(got->g+1),ByteCap(got->b-1));

                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-0),ByteCap(got->b-1));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g+0),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-0),ByteCap(got->b+1));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g-0),ByteCap(got->b-1));
                
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-1),ByteCap(got->b-0));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g+1),ByteCap(got->b+0));
                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g+1),ByteCap(got->b-0));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g-1),ByteCap(got->b-0));

                printf("%u %u %u\n",ByteCap(got->r-0),ByteCap(got->g-0),ByteCap(got->b-1));
                printf("%u %u %u\n",ByteCap(got->r+0),ByteCap(got->g+0),ByteCap(got->b+1));

                printf("%u %u %u\n",ByteCap(got->r-1),ByteCap(got->g-0),ByteCap(got->b-0));
                printf("%u %u %u\n",ByteCap(got->r+1),ByteCap(got->g+0),ByteCap(got->b+0));
                
                printf("%u %u %u\n",ByteCap(got->r-0),ByteCap(got->g-1),ByteCap(got->b-0));
                printf("%u %u %u\n",ByteCap(got->r+0),ByteCap(got->g+1),ByteCap(got->b+0));
          }
        }
        
	if (dither == 1) error_diffuse(im, &heap);
	else if (dither == 0)
